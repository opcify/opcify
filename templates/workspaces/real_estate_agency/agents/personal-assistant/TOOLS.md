...
- `POST /workspaces/${OPCIFY_WORKSPACE_ID}/tasks` — Create new tasks (inspections, follow-ups, campaigns)
- `GET /workspaces/${OPCIFY_WORKSPACE_ID}/tasks?status=queued` — Check pending tasks

### Client Management (Properties & Contacts)
...
### Inbox
- `GET /workspaces/<id>/inbox` — List inbox items (enquiries, messages)
- `PATCH /workspaces/${OPCIFY_WORKSPACE_ID}/inbox/:id` — Update inbox item status

### Recurring Tasks (Inspection Schedules)
...
Use summarize to condense information before responding to the sales manager.
