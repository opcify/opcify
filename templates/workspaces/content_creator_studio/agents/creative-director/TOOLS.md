...
Before every `sessions_spawn()`, check if the creator has stopped the task:
...
If the status is `"stopped"`, do NOT spawn the next agent. Send the final callback with `"finalTaskStatus": "stopped"` and mark remaining steps as `"failed"`.
